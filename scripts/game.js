	...
	return function game() {
		...
		this.initialize = function(io) {
			...
			io.sockets.on('connection', function(socket) {
				...
				self.setPlayer(socket);

				socket.emit('playersTurn', playersTurn);

				socket.emit('state', getStateJSON());
				...
				socket.on('updatePiece', function(row, col, playerNum) {
					...
					if (state[row][col] === 0 && playersTurn == playerNum) {
						...
						if (self.isGameOver(row, col, playerNum)) {
							console.log("------------------------------");
				...
				socket.on('newGame', function() {
					resetState();
					io.sockets.emit("state", getStateJSON());
				});
		...
		function getStateJSON() {
			...
			for (var i = 0; i < state.length; i++) {
				...
				for (var j = 0; j < state[0].length; j++) {
					row.push({row: i, col: j, data: state[i][j]});
				}
				json.push(row);
			}
